...
validateCommitMessages() {
    ...
    COMMIT_MESSAGE_REGEX="(((VKIDSDK|ESDK)-[0-9]+: [A-Z][a-zA-Z0-9';,: -]+))|(Merge branch .*)"
    ...
    git log --pretty=oneline $TARGET_BRANCH_SHA..$SOURCE_BRANCH_SHA | while read line ; do
        if ! grep -iqE "$COMMIT_MESSAGE_REGEX" <<< "$line"; then
            echo "Commit message "$line" must match '(VKIDSDK|ESDK)-XXX: Commit description pattern or be a merge commit'"
            exit 1
        fi
    done
...
validateBranchName() {
    ...
    BRANCH_NAME_REGEX='((epic|task|fix|feature)\/(VKIDSDK|ESDK)-[0-9]+\/([a-zA-Z0-9-]+)*|(release/.*)|(master))'
    error_msg="Branch name must match 'type/(VKIDSDK|ESDK)-XXX/branch-name'"
